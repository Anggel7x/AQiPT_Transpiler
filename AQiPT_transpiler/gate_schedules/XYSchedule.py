...
class XYSchedule(GateSchedule):
    ...
    def _schedule(self):
        ...
        p2 = SquarePulse(t_start=p1.t_end, area=2*np.pi/omega2, backend=self.backend_config)

        # 3: Same as before but phase -Pi
        p3 = SquarePulse(t_start=p2.t_end*1.2, area=2*np.pi/omega2, backend=self.backend_config)
        ...
        p4 = ShapedPulse(t_start=p3.t_end, area=np.pi/omega1, backend=self.backend_config)
