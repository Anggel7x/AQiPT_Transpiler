...
def dec2bin(val):
    ...
    s = bin(val)[
        2:
    ]  # Ésto remueve el prefijo "0b" del string que devuelve la función "bin"
    return s[::-1]
...
def swapping(qc):
    ...
    for qubit in range(n // 2):
        qc.swap(
            qubit, n - qubit - 1
        )  # Hacemos el swap hasta llegar a la mitad entera, si hay 3 qubits entonces éste sera 1
    return qc
...
def Bjk(qc, n, barrierON=True):
    ...
    for qubit in range(qc.num_qubits - n, qc.num_qubits):
        qc.cp(
            2 * np.pi / 2 ** (qubit - (qc.num_qubits - n - 1) + 1),
            qubit,
            qc.num_qubits - n - 1,
        )  # k=qubit, j=qc.num_qubits-(n+1)
...
def QFT_IBM(state, n, barrier=True):
    ...
    for i in range(len(bin_x)):
        if bin_x[i] == "1":
            qc.x(i)
            # adding initial rotations

    ContRot(qc, n, barrier)
    # adding control rotations
    swapping(qc)
    # swapping qubits
    return qc
